<template>
  <div class="userHome">
    ...
    <div id="userHome" v-if="!checkVotingInfo_show">
      <el-row style="margin:20px">
        <el-col :span="6" v-for="(item, key) in headItems" :key="key">
          <v-card
            width="95%"
            height="170px"
            style="margin:0 2px">
            <el-row>
              <el-col>
                <h1 style="text-align:center; line-height:68px; background-color:green">
                  {{ item.text }}
                </h1>
              </el-col>
            </el-row>
            <el-row>
              <el-col :span="12">
                <v-card 
                  ...
                  width="100"
                  color="grey darken-3"
                  style="font-size:48px; margin:15px 15px; text-align:center; color:white">
                  {{ item.num }}
                </v-card>
              </el-col>
              <el-col :span="12">
                <v-btn
                  height="50"
                  width="115"
                  style="font-size:24px; margin:25px auto"
                  to="/User/VotingInfo">查看更多</v-btn>
              </el-col>
            </el-row>
          </v-card>
        </el-col>
      ...
      <el-row style="margin:0 20px; border:2px solid black">
        <el-col>
          <el-table
            :data="voteDataTable"
            style="width: 100%">
                ...
                <div>
                  <span v-if="props.row.status=='进行中'">
                    <v-btn color="blue" width="80" height="30"
                          @click="checkVotingInfo_show=!checkVotingInfo_show;
                                  showVoteInfo(props.row.voteID, '4')">查看</v-btn>
  ...
  methods: {
    getVotingData() {
      ...
      this.$http.post(
        ...
        this.voteDataTable = res.data.result
      })
    },
    ...
    getVoteNum() {
      ...
      this.$http.post(
        ...
        let len1 = res.data.result.length
        this.headItems[0].num = len1
        ...
        this.$http.post(
          '/api/selectVoteData',
          {
            ...
            let len2 = res.data.result.length
            this.headItems[1].num = len2
            ...
            this.$http.post(
              '/api/selectVoteData',
              {
                ...
                let len3 = res.data.result.length
                this.headItems[2].num = len3
                ...
                this.$http.post(
                  '/api/selectVoteData',
                  {
                    ...
                    let len4 = res.data.result.length
                    this.headItems[3].num = len4
                })
            })
        })
      })
